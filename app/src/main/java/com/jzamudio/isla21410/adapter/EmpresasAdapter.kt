...
class EmpresasAdapter(private val listEmpresa:List<Empresa>) : RecyclerView.Adapter<EmpresasViewHolder>() {

    //Infla la vista
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): EmpresasViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)

        return EmpresasViewHolder(layoutInflater.inflate(R.layout.item_empresas_adapter,parent,false))
    }
    ...
    override fun onBindViewHolder(holder: EmpresasViewHolder, position: Int) {
        val item = listEmpresa[position]
        holder.render(item)
    }

    //Devuelve el numero de elementos
    override fun getItemCount(): Int = listEmpresa.size
